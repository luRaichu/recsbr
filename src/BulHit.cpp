...
int JudgeHitBulletBlock(int x, int y, BULLET *bul)
{
	...
	if (hit && bul->bbits & 0x60 && GetAttribute(x, y) == 0x43)
	{
		...
		SetCaret(bul->x, bul->y, 12, 0);
		SetCaret(bul->x, bul->y, 18, 0);
		SetCaret(bul->x, bul->y, 18, 0);
		SetCaret(bul->x, bul->y, 18, 0);
		SetCaret(bul->x, bul->y, 18, 0);
		SetCaret(bul->x, bul->y, 18, 0);
		...
		for (i = 0; i < 4; ++i)
			SetNpChar(3, x * 0x200 * 0x10, y * 0x200 * 0x10, Random(-0x200, 0x200), Random(-0x200, 0x200), 0, NULL, 0x100); //I think this is for snack blocks //4
			PlaySoundObject(07, SOUND_MODE_PLAY); //dummy dumb dumb
		ShiftMapParts(x, y);
	}
